\documentclass[11pt]{amsart}
\usepackage[margin=0.9in]{geometry}                % See geometry.pdf to learn the layout options. There are lots.
\geometry{letterpaper}                   % ... or a4paper or a5paper or ... 
%\geometry{landscape}                % Activate for for rotated page geometry
%\usepackage[parfill]{parskip}    % Activate to begin paragraphs with an empty line rather than an indent
\usepackage{graphicx}
\usepackage{amssymb}
\usepackage{epstopdf}
\usepackage{multicol}
\DeclareGraphicsRule{.tif}{png}{.png}{`convert #1 `dirname #1`/`basename #1 .tif`.png}

\usepackage{hyperref}
\hypersetup{
    colorlinks,
    citecolor=black,
    filecolor=black,
    linkcolor=black,
    urlcolor=black
}


\author{}
\begin{document}
\pagestyle{empty}
\setlength\parindent{0pt}

\Huge
\hspace*{\fill} \textbf{James Macek} \hspace*{\fill}
\normalsize
\vspace{1cm}

 \begin{tabular}{p{5cm} p{1cm} p{10cm}}
 \textbf{Address:}       &  & \textbf{Phone:} +1-647-458-9797                                                                                        \\
 Alberta School of Business &  & \textbf{Email:} \href{mailto:jmacek@ualberta.ca}{jmacek@ualberta.ca}                         \\
 University of Alberta  &  & \textbf{Website:} \href{https://jamesmacek.github.io}{jamesmacek.github.io}  \\
11211 Saskatchewan Dr NW       &  &                                                                                                                        \\
Edmonton, Alberta        &  &                                                                                                                        \\
T6G 2R3, Canada          &  &                                                                                                                        
\end{tabular}

\line(1,0){470}


 \begin{tabular}{ p{4cm}  p{15cm}}
 \textbf{Citizenship:}           & Canadian                                     \\
                                 &                                              \\
\textbf{Research Interests:}     & Urban \& Real Estate economics, Spatial economics  \\ 
                                 &                                           \\
\end{tabular}



\vspace{0.2cm}





\LARGE
\textsc{Employment}
\vspace{0.2cm}

\normalsize
\begin{tabular}{ p{12.5cm}  p{5cm}}
	\large{Assistant Professor of Business Economics, Alberta School of Business}  & 2025-  \\
 
\end{tabular}

\vspace{0.2cm}

\LARGE
\textsc{Education}
\vspace{0.2cm}

\normalsize
\begin{tabular}{ p{12.5cm}  p{5cm}}
  \large{Ph.D. in Economics, University of Toronto}  & 2025  \\
   \multicolumn{2}{l}{ \hspace{.75cm}  \emph{Committee:}  Nathaniel Baum-Snow (supervisor), } \\
     \hspace{2.8cm}  William Strange, Kevin Lim, Joseph Steinberg  \\
 \hspace{2.90cm}   \\
  \\
  \large{M.A in Economics, University of Toronto}    & 2019            \\
   \\
  \large{Bachelor of Business Administration, University of Toronto Scarborough}   & 2018            \\
                                                     &                 
\end{tabular}

\vspace{0.2cm}


%Publications go here 
%%%%%%%%%% WORKS IN PROGRESS %%%%%%%%%%%
\LARGE
\textsc{Publications}
\vspace{0.2cm}
\normalsize

\begin{tabular}{ p{15.5cm}}
	\textbf{Housing Supply and Affordability in Canada} (with Nathaniel Baum-Snow and Aled Ab Iorwerth) \\ 
	\textit{Canadian Journal of Economics}, 2026                         \\
	\normalsize
\end{tabular}

\vspace{0.2cm}


\LARGE
%%%%%%%%%% WORKING PAPERS %%%%%%%%%%%
\textsc{Working Papers}
\vspace{0.2cm}
\normalsize

\begin{tabular}{ p{15.5cm}}
 \href{https://jamesmacek.github.io/research/lswus}{\textbf{Housing Regulation and Neighborhood Sorting across the United States}}                         \\

\\
 \href{https://jamesmacek.github.io/research/gent}{\textbf{Gentrification and Redevelopment in General Equilibrium}} (with Guangbin Hong) \\
\\
\normalsize
\end{tabular}

\vspace{0.2cm}

\LARGE
\textsc{Works in Progress}
\vspace{0.2cm}
\normalsize

\begin{tabular}{ p{15.5cm}}
	\textbf{Segmented Housing Supply} (with Nathaniel Baum-Snow and Sidharth Moktan)                        \\
	
	\\
	
	\textbf{Thinking Outside the Lot: Historical Parcel Boundaries and Housing Supply} (with Devin Bissky-Dziadyk and Stephan Heblich) 

	\normalsize
\end{tabular}

%%%%%%%%%%% WORKS IN PROGRESS %%%%%%%%%%%
%\LARGE
%\textsc{Selected Works in Progress}
%\vspace{0.2cm}
%\normalsize

%\begin{tabular}{ p{15.5cm}}
%	\textbf{Housing Supply and Affordability in Canada} (with Nathaniel Baum-Snow and Aled Ab Iowerth)                         \\
%	\normalsize
%\end{tabular}

\vspace{0.2cm}

\LARGE
\textsc{Awards and Grants}
\vspace{0.2cm}

\normalsize 
\begin{tabular}{ p{12.5cm}  p{5cm}}
  SSHRC Connection Grant (\$12,500, with Nathan Schiff and Jay Hyun), University of Alberta Startup Grant (\$15,000) & 2026 \\ 
  University of Alberta Startup Grant (\$15,000) & 2025 \\
  Bank of Canada Best Student Paper (\$2500), Honorable Mention for Best Student Paper, Urban Economics Association European Meeting,   Ontario Graduate Scholarship (\$5000 $\times$ 3),  AREUEA Travel Grant (\$1300), University of Toronto Travel Grants (\$1500) & 2024 \\
  \vspace{1mm}
  University of Toronto Doctoral Fellowship  (\$12,000 $ \times$ 5) &   \vspace{1mm} 2019-2024 \\
  \vspace{1mm}
  Yurgen Krumma Award in Economics &   \vspace{1mm} 2017
\end{tabular}

\vspace{0.4cm}

\LARGE
\textsc{Teaching}
\vspace{0.2cm}
\normalsize

\begin{tabular}{ p{12.5cm} p{5cm}}
Instructor & 2025- \\
\begin{itemize}
	\item BUEC 211: Business Economics, Organizations, and Management (Microeconomic Theory for Business Students)
\end{itemize} & \\

Teaching Assistant & 2017 - 25 \\
\begin{itemize}
  \item ECO 204: Microeconomics for Commerce
  \item ECO 231: Economics of Global Trade
  \item ECO 380: Markets, Competition and Strategy
  \item ECO 220: Data Analysis and Applied Econometrics
\end{itemize} 
                                  
\end{tabular}





\normalsize

\vspace{0.4cm}
\LARGE
\textsc{Conference and Seminar Presentations \scriptsize \\ (including scheduled or by co-author)}
\vspace{0.2cm}
\normalsize

\begin{tabular}{ p{12.5cm} p{5cm}}
	
Notre Dame Housing Economics Research Conference (South Bend), University of Colorado Boulder, Spring Midwest Macroeconomics (Milwaukee), AREUEA National Conference (Washington, DC), Kelley-Chicago Fed Third Symposium on Real Estate and Urban Policy (Indianapolis), Organizer for the Rocky Mountain Urban and Real Estate Conference (Banff), North American Meeting of the Urban Economics Association (Chicago), York University  & 2026\\

University of Saskatchewan, Alberta School of Business, UPenn Wharton Real Estate (canceled), CES North American Meeting (Ann Arbor), European Meeting of the Urban Economics Association (Berlin), Midwest Macroeconomics Meeting (Kansas Fed), AREUEA National Conference (Washington, DC), NBER Summer Institute (Real Estate), North American Meeting of the Urban Economics Association (Montreal), Institute Research Conference (Minneapolis Fed)   & 2025 \\	
	
Bank of Canada Student Award Finalist Conference (winner), Urban Economics Association Summer School (Geneva), European Meeting of the Urban Economics Association (Copenhagen), University of British Columbia Sauder CUERE Symposium, Annual Conference of the Canadian Economics Association (Toronto),  AREUEA Doctoral Poster Session   & 2024  \\	

\end{tabular}


\vspace{0.4cm}

\LARGE
\textsc{Refereeing Experience}
\vspace{0.2cm}
\normalsize

\begin{tabular}{ p{17.5cm} }
Journal of Urban Economics (x4), American Economic Journal: Economic Policy \\
\end{tabular}
 

\vspace{0.4cm}
\LARGE
\textsc{Academic Service}
\vspace{0.2cm}

\normalsize
\begin{tabular}{ p{12.5cm} p{5cm}}

Co-Organizer of the Rocky Mountain Urban and Real Estate Conference (with Jay Hyun and Nathan Schiff)  & 2026- \\


Co-Organizer of the Graduate Student Workshop  & 2023 - 2024  \\
\end{tabular}

\vspace{0.4cm}
\LARGE
\textsc{Languages}
\vspace{0.2cm}

\normalsize
\begin{tabular}{ p{17.5cm}}
  English (native)  \\
  \emph{Programming:} R, Stata, Python, SQL, MATLAB, GIS (using R, Python and QGIS)
\end{tabular}




%Refereed For

\vspace{0.5cm}
\begin{center}
\tiny Last Updated: \today
\end{center}


\end{document}  